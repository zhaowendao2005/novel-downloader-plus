...
class AsyncChapterDownloader:
    ...
    def _download_single_chapter(self, chapter, selector_content):
    ...
    def download_chapters(self, chapters, selector_content):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_chapter = {
                executor.submit(self._download_single_chapter, chapter, selector_content): chapter
                for chapter in chapters
            }
            for future in concurrent.futures.as_completed(future_to_chapter):
                title, content, error = future.result()
                if error:
